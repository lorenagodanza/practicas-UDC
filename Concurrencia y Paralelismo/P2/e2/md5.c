...
void sum(struct options opt) {
    ...
    // Crear cola de entrada y de salida
    in_q  = q_create(opt.queue_size);
    out_q = q_create(opt.queue_size);
    struct read_files_args args = {opt.dir, in_q};

    // Crear hilo para obtener la lista de archivos
    ...
    // Escribir hashes en el archivo de salida
    ...
    // Esperar a que el hilo de obtener la lista de archivos termine
