...
void *read_hash_file_thread(void *arg) {
    struct read_hash_file_args *args = arg;
    read_hash_file(args->file, args->dir, args->q);
    q_set_finished(args->q);
    return NULL;
}

struct check_files_args {
    char *file;
    char *dir;
    queue q;
    int num_threads;
...
void *check_files_thread(void *arg) {
    struct check_files_args *args = arg;
...
void check(struct options opt) {
    ...
    // Crear hilo para la lectura del archivo de hash
    struct read_hash_file_args read_args = {opt.file, opt.dir, in_q};
    thrd_t thread_read_hash_file;
    thrd_create(&thread_read_hash_file, (void *) read_hash_file_thread, &read_args);

    // Crear hilos para calcular los hashes de los archivos a comprobar
    thrd_t threads[opt.num_threads];
    struct check_files_args check_args = {opt.file, opt.dir, in_q, opt.num_threads};
    for(int i = 0; i < opt.num_threads; i++) {
        thrd_create(&threads[i], (void *) check_files_thread, &check_args);
    }

    // Esperar a que todos los hilos de cálculo de hashes terminen
    for(int i = 0; i < opt.num_threads; i++) {
        thrd_join(threads[i], NULL);
    }

    // Esperar a que el hilo de lectura de hash termine
    thrd_join(thread_read_hash_file, NULL);
