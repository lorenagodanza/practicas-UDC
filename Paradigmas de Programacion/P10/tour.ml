...
let tour m n trees salto=
   let rec aux pila=
      if pila=[] then raise Not_found 
      else 
         let visitados,(x,y)=List.hd pila, List.hd(List.hd pila) in
            if (x,y)=(m,n) then List.rev visitados
            else 
              let posibilidades=get_posibilidades visitados trees (x,y) salto in
               let pila2=(List.map (fun x->x::visitados)posibilidades)@(List.tl pila) in aux pila2
           in aux [[(1,1)]];;
